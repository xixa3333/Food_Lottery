...
- 預設只抽目前營業中且超過 30 分鐘才打烊的店家；可分別勾選包含即將打烊、目前休息／已打烊，或無法確認營業狀態的店家。
- 首次搜尋若 Google 回傳滿 20 家，程式會依半徑自動分區搜尋並以 Place ID 去重，再統一篩選；「在此清單重抽」只使用記憶體中的候選清單，不會增加 API 請求。
- 餐飲類型選擇器收錄 Google Places Table A 的全部 166 種 Food and Drink 類型，支援搜尋、全選、清除與常用類型；Nearby Search 會依官方限制每 50 種分批。
