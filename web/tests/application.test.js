...
test("manual location resolves through Places service and selects eligible result", async () => {
  const calls = [];
  const places = { resolveLocation: async q => (calls.push(q), { center: { lat: 25, lng: 121 }, label: q }), searchRestaurants: async () => [{ displayName: "店", location: { lat: 25, lng: 121 } }] };
...
test("device location skips manual resolver and adds bounded accuracy", async () => {
  let resolved = false;
  const places = { resolveLocation: async () => (resolved = true), searchRestaurants: async () => [{ displayName: "店", location: { lat: 25.001, lng: 121 } }] };
